...
always @(posedge clk or negedge rst_n)
  begin
    if(rst_n == 1'b0)
      fifo_rd_en <= 1'b0 ;
    // udp_send_cnt = n 处的载荷字节,是由**早 4 拍**发出的那次读提供的
    // (FIFO 标准模式读延迟 1 + ram_rdata_d0 1 + …… 见 dout 那级流水)。
    // 第一个载荷字节在 cnt 8,所以第一次读在 cnt 4;最后一个载荷字节在 cnt (total-1),
    // 所以最后一次读在 cnt (total-5),即 cnt < total-4。
    // 参考工程用的是 total-3:对 1024 字节的载荷会发出 1025 次读,于是每个包都把数据流
    // 错开一个字节(累积到每帧约 506 像素的水平错位)。
    else if (state == UDP_SEND && udp_send_cnt > 16'd3 &&
             udp_send_cnt < udp_total_data_length - 16'd4)
      fifo_rd_en <= 1'b1 ;
    else
      fifo_rd_en <= 1'b0 ;
  end
